...
SIGN_KEY_FILE=""
SIGN_KEY_PASS=""
# Optional external CoT keys. When set, these are forwarded to cert_create
# so no non-root key is auto-generated during the build.
TRUSTED_WORLD_KEY_FILE="${TRUSTED_WORLD_KEY:-}"
NON_TRUSTED_WORLD_KEY_FILE="${NON_TRUSTED_WORLD_KEY:-}"
SOC_FW_KEY_FILE="${BL31_KEY:-}"
TOS_FW_KEY_FILE="${BL32_KEY:-}"
NT_FW_KEY_FILE="${BL33_KEY:-}"
SCP_FW_KEY_FILE="${SCP_BL2_KEY:-}"
USE_BL31=0
...
if [ $USE_ONLY_DDR -eq 1 ]; then
    ...
    if [ $NEED_TO_SIGN -eq 1 ]; then
        ...
        cot_key_opt=""
        [ -n "${TRUSTED_WORLD_KEY_FILE}" ] && cot_key_opt="$cot_key_opt --trusted-world-key ${TRUSTED_WORLD_KEY_FILE}"
        [ -n "${NON_TRUSTED_WORLD_KEY_FILE}" ] && cot_key_opt="$cot_key_opt --non-trusted-world-key ${NON_TRUSTED_WORLD_KEY_FILE}"
        [ -n "${SOC_FW_KEY_FILE}" ] && cot_key_opt="$cot_key_opt --soc-fw-key ${SOC_FW_KEY_FILE}"
        [ -n "${TOS_FW_KEY_FILE}" ] && cot_key_opt="$cot_key_opt --tos-fw-key ${TOS_FW_KEY_FILE}"
        [ -n "${NT_FW_KEY_FILE}" ] && cot_key_opt="$cot_key_opt --nt-fw-key ${NT_FW_KEY_FILE}"
        [ -n "${SCP_FW_KEY_FILE}" ] && cot_key_opt="$cot_key_opt --scp-fw-key ${SCP_FW_KEY_FILE}"

        echo "CMD> ${TOOLS_CERTTOOL} -n --tfw-nvctr 0 --ntfw-nvctr 0 --key-alg ecdsa --hash-alg sha256 --rot-key ${sign_key} --rot-key-pwd ${sign_single_key_pass} ${cot_key_opt} ${certool_opt} ${fip_certconf_opt} ${fip_certconf_opt_addons} --tb-fw ${fiptool_output_temp_file}/bl2-fake.bin" | sed 's| --| \\\n\t--|g'

        if [ $DRY_RUN -eq 0 ]; then
            ${TOOLS_CERTTOOL} -n --tfw-nvctr 0 --ntfw-nvctr 0 --key-alg ecdsa --hash-alg sha256                 --rot-key ${sign_key}                 --rot-key-pwd $sign_single_key_pass                 $cot_key_opt                 $certool_opt                 $fip_certconf_opt                 $fip_certconf_opt_addons                 --tb-fw $fiptool_output_temp_file/bl2-fake.bin || die "CERTOOL error"
        fi
    fi
fi
